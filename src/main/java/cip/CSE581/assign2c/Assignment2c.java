...
public class Assignment2c implements IRenderScene {
    ...
    Color getPixelColor(Line3f ray) {
        ...
        try {
            intersection.m_vToEve.setValue(ray.m_vDir).reverse();
            ...
            if (bIntersectObj) {
                intersection.m_mtl.getColor(rgb, intersection, m_lights,
                        m_geometry, m_rgbBkg);
//                // Loop through the lights and add the illumination components.
...
//                                            scale(NdotL).scale(intersection.m_mtl.m);
//                                    rgb.add(rgbDiffuse);
//                                }
//                            }
//                        }
                ...
                if (lightInfo.m_nType == LightInfo.AMBIENT) {
                    ...
                    if (!bInShadow) {
                        ...
                        if (NdotL > 0.0f) {
                            ...
                            final Vector3f vH = new Vector3f(rayLight.m_vDir).add(intersection.m_vToEve).normalize();
...
class RayIntersection {
    ...
    public Vector3f    m_vToEve;      // the direction to the eye
...
class SpotLight implements IRtLight {
    ...
    private float m_fDimmer = 1.0f;                 // the dimmer value
    private final Point3f m_location;
    private final Vector3f m_directiuon;
    private final AngleF m_beta;
    // derived values
